...
export const authedProcedure = t.procedure.use(async function isAuthed(opts) {
  ...
  const sessionId = getCookie(opts.ctx.event.nativeEvent, lucia.sessionCookieName) ?? null;
  ...
  if (sessionCookie) {
    opts.ctx.event.response.headers.append('Set-Cookie', sessionCookie.serialize())
  }
